...
{ 
    ...
    { 
        ...
        if(X >= 0 && X <= x_len_map * 50 && Y >= 0 && Y <= y_len * 50)
        {
            if(swiitch == 0)
...
int wall_checker()
{
    ...
    //printf("\n|==>|'%c'||\n",line[Y_pos][X_pos]);
    if ((X_pos >= 0 &&  X_pos < x_len_map) && (Y_pos >= 0 &&  Y_pos < y_len))
    {
...
int ft_cast_ray(int index)
{
    ...
    while(nextHoriX >= 0 && nextHoriX < x_len_map * 50 && nextHoriY >= 0 && nextHoriY < y_len * 50)
    {
        sprite = 0;
        check = wall_checker();
        // if(sprite_count > 6)
